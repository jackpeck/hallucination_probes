...
def evaluate_probe(
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        probe_logits: Float[Tensor, "batch_size seq_len"] = outputs[
            "probe_logits"
        ].squeeze(-1)
        probe_probs: Float[Tensor, "batch_size seq_len"] = torch.sigmoid(
            probe_logits
        ).float()
        probe_preds: Float[Tensor, "batch_size seq_len"] = (
            probe_probs > threshold
        ).float()
    ...
    for agg_level in ["all", "span", "span_max"]:
        ...
        clf_metrics = compute_clf_metrics(
            all_preds[agg_level], all_labels[agg_level], all_probs[agg_level]
        )
